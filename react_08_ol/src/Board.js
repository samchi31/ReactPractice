...
export default function Board(props) {  
    ...
    // console.log("props1", map.getAllLayers());
    // console.log("state2", mapState.getAllLayers());
    ...
    useEffect(() => {
      console.log("자식 렌더링 후");
    });
    console.log("자식 렌더링 전");
  
    useEffect(() => {
      console.log("자식 props 변경");
    }, [props.mapState]);
  
    useEffect(() => {
      console.log("자식 map set ", props.mapState.getAllLayers())
      // setMapState(props.mapState);
      if(!mapState) {
        console.log("자식 null 분기문 ", props.mapState.getAllLayers(), mapState)
        return;
      }
